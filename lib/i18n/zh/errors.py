MESSAGES = {
    ...
    "prompt_partial_not_found": "提示词片段 '{id}' 不存在",
    ...
    "reference_prompt_preview_missing": "该单元还没有填写正文",
    ...
    "task_fail_comfyui_job_lost": "ComfyUI 上找不到这次执行（{prompt_id}），队列与历史里都没有它，ComfyUI 多半重启过，请重试",
    ...
    "task_fail_comfyui_output_type_mismatch": "ComfyUI 产出的 {filename} 不是这个端点应产出的文件类型（应为 {expected}），请把产物节点的节点绑定指到导出最终产物的那个节点，或在该保存节点上改用受支持的格式（如 SaveVideo 选 h264-mp4）",
    "task_fail_comfyui_output_container_mismatch": "ComfyUI 产出的 {filename} 文件内容不是这个端点应产出的容器（应为 {expected}），请在保存节点上改用受支持的格式（如 SaveVideo 选 h264-mp4）",
    ...
    # ---- 市场 ----
    "market_source_not_found": "市场源不存在",
    "market_source_official_undeletable": "官方市场源不可删除，可以禁用",
    "market_source_duplicate": "该市场源已添加",
    "market_source_address_empty": "请填写市场源地址",
    "market_source_address_insecure_scheme": "市场源地址必须使用 https://",
    "market_source_address_unsupported": "无法识别的市场源地址：请填写 owner/repo、owner/repo@ref、GitHub 仓库地址，或以 arcreel-market.json 结尾的 https 直链",
    "market_source_fetch_failed": "无法添加市场源：{status}（{reason}）",
    "market_source_status_never_fetched": "尚未刷新",
    "market_source_status_ok": "正常",
    "market_source_status_unreachable": "无法访问",
    "market_source_status_invalid_index": "索引无效",
    "market_source_status_unsupported_schema": "索引格式版本过新，需升级应用",
    "market_source_display_name_required": "显示名不能为空",
